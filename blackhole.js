 ...
 *   吞噬 → M += m(方程 7)、耀发 ∝ m(方程 12)、铃宕叠加(方程 8)。
 */
...
function init(opts = {}) {
  ...
  function frame(tMs) {
    ...
    const ring = ringdownOffset(simT - simTLastSwallow, rsShown, cfg.cPx, ringAmp);
    flare *= Math.exp(-dt / 0.8);

    renderer.render({
      cx: center.x, cy: center.y,
      rsPx: Math.max(0.01, rsShown + ring),
      flare,
  ...
  return {
    ...
    get state() {
      return { active, rs: rs(), swallowed, totalTargets, mode: renderer.mode };
    },
  };
}
